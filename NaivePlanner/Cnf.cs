...
namespace NaivePlanner
{
    ...
    public class Cnf_old
    {

        private List<SortedSet<int>> _clauses = new List<SortedSet<int>>();
        public IReadOnlyList<SortedSet<int>> Clauses => _clauses;

        private SortedSet<int> _unitClauses = new SortedSet<int>();
        private SortedSet<int> _emptyClauses = new SortedSet<int>();

        private SortedDictionary<int, SortedSet<int>> _literalsToClauses = new SortedDictionary<int, SortedSet<int>>();
        public IReadOnlyDictionary<int, SortedSet<int>> ClausesWithLiteral => _literalsToClauses;

        private SortedDictionary<int, SortedSet<int>> _variablesToClauses = new SortedDictionary<int, SortedSet<int>>();
        public IReadOnlyDictionary<int, SortedSet<int>> ClausesWithVariable => _variablesToClauses;

        public ICollection<int> Variables => _variablesToClauses.Keys;
        public ICollection<int> Literals => _literalsToClauses.Keys;
        public ICollection<int> UnitClauses => _unitClauses;
        public ICollection<int> EmptyClauses => _emptyClauses;
        ...
        public Cnf_old(IEnumerable<IEnumerable<int>> clauses)
        {
            foreach (var clause in clauses)
            {
                AddClause(clause);
        ...
        public void AddClause(IEnumerable<int> literals)
        {
            var clause = new SortedSet<int>();
            foreach (var l in literals)
            {
                if (clause.Add(l))
                {
                    if (clause.Count == 1)
                    {
                        _unitClauses.Add(_clauses.Count);
                    }
                    else
                    {
                        _unitClauses.Remove(_clauses.Count);
                    }



                    if (!_literalsToClauses.TryGetValue(l, out var clausesWithLiteral))
                    {
                        clausesWithLiteral = new SortedSet<int>();
                        _literalsToClauses[l] = clausesWithLiteral;
                    }
                    clausesWithLiteral.Add(_clauses.Count);



                    var v = (l < 0) ? -l : l;
                    if (!_variablesToClauses.TryGetValue(v, out var clausesWithVariable))
                    {
                        clausesWithVariable = new SortedSet<int>();
                        _variablesToClauses[v] = clausesWithVariable;
                    }
                    clausesWithVariable.Add(_clauses.Count);
        ...
        public bool CheckAssignment(IEnumerable<bool> assignment, out SortedSet<int> unsatisfiedSet)
        {
            ...
            foreach (var a in assignment)
            {
                if (_variablesToClauses.TryGetValue(v, out var clauses))
                {
                    foreach (var c_i in clauses)
                    {
                        if (unsatisfiedSet.Contains(c_i) && Clauses[c_i].Contains(a ? v : -v))
                        {
                            unsatisfiedSet.Remove(c_i);
                            if (unsatisfiedSet.Count == 0)
                            {
        ...
        static public Cnf_old LoadFile(string filename)
        {
            var fi = new System.IO.FileInfo(filename);
            if (!fi.Exists)
            {
        ...
        static public Cnf_old LoadStream(Stream stream)
        {
        ...
        public Cnf_old Copy()
        {
            var newCnf = new Cnf_old();
            foreach (var clause in Clauses)
            {
                newCnf.AddClause(clause);
            }
            return newCnf;
        }
